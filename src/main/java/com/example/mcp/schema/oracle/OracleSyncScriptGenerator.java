...
import com.example.mcp.schema.ConstraintDiff;
import com.example.mcp.schema.DiffType;
import com.example.mcp.schema.IndexDiff;
import com.example.mcp.schema.RoutineDiff;
import com.example.mcp.schema.SchemaDiffResult;
import com.example.mcp.schema.SequenceDiff;
import com.example.mcp.schema.SyncScriptGenerator;
import com.example.mcp.schema.TableDiff;
import com.example.mcp.schema.ViewDiff;
import java.util.ArrayList;
import java.util.List;
import java.util.Objects;

public class OracleSyncScriptGenerator implements SyncScriptGenerator {
   @Override
   public List<String> generateScripts(SchemaDiffResult result) {
               ...
               for (ColumnDiff colDiff : diff.columnDiffs()) {
                  switch (colDiff.type()) {
                     case MISSING_IN_TARGET -> {
                        ...
                        scripts.add("ALTER TABLE " + fullTableName + " ADD (" + quoteIdentifier(colDiff.columnName()) + " " + colDiff.sourceType() + ");");
                     }
                     case MISSING_IN_SOURCE -> {
                     ...
                     case DIFFERENT -> {
                        if (!Objects.equals(colDiff.sourceType(), colDiff.targetType())) {
                           scripts.add("-- modify column type: " + diff.objectName() + "." + colDiff.columnName());
                           scripts.add("ALTER TABLE " + fullTableName + " MODIFY (" + quoteIdentifier(colDiff.columnName()) + " " + colDiff.sourceType() + ");");
                        }
                     }
                  }
               }
      ...
      for (RoutineDiff diff : result.routineDiffs()) {
         scripts.add("-- review routine difference manually: " + diff.objectName());
      }
